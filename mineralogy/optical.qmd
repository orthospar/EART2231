...
<!-- d <- data.frame(wavelength=c(380:750))%>% #wavelength in nm -->
<!--           mutate(freq = (c/(wavelength*1e-9))/1e12) #calculate frequency and express in THz -->
<!-- wave <- function(freq, time, phi){ -->
<!--   y <- sin(2*pi*freq*time + phi) -->
<!--   return(y) -->
<!-- } -->

<!-- temp <- data.frame(time = c(0:5*pi)) -->


<!-- p <- ggplot(temp, aes(x=time))+ -->
<!--   geom_function(fun = ~sin(2*pi*500*.x)) -->
...
//data_colours = FileAttachment("bb_colours.csv").csv({ typed: true })
```
...
//colour_vis = data_colours.filter(function(bbcolour) {
//  return TempK == bbcolour.temperature;
//})
```
